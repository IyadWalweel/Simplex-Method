...
def simplex(c, Al = None, bl = None, Ag = None, bg = None, Ae = None, be = None, obj = 'Min', Show = False, Big_M = False, M=100):
    ...
    if (case is not None):
        ...
        x = np.zeros(lc)
        table, x_new, C, basic_phs, I, Entering, EI, Xs = phase1(c, table, x, obj, lengths)
        if (table[1][-1] > 0):
            print("No Feasible Solution")
            result, table, X, Y, F, ite = None, None, None, None, None, None
            # return result, table, X, Y, F, ite
        else:
            result, table, X, Y, F, ite = phase2(c, table, x_new, obj, C, basic_phs, I, lengths, Entering, EI, Xs)
            # return result, table, X, Y, F, ite
    if Show:
